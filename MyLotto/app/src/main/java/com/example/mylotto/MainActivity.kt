...
class MainActivity : AppCompatActivity() {
    ...
    private fun initAddButton(){
        addButton.setOnClickListener{
            ...
            when {
                didRun -> showToast("초기화 후에 실행해 주세요.")
                pickNumSet.size >= 5 -> showToast("숫자는 최대 다섯 개까지 선택할 수 있습니다..")
                pickNumSet.contains(numPick.value) -> showToast("이미 선택 된 숫자입니다.")
                else -> {
                    val textView = numTextViewList[pickNumSet.size]
                    textView.isVisible = true;
                    textView.text = numPick.value.toString();

                    setNumBack(numPick.value.toInt(), textView)
    ...
    private fun getRandom() : List<Int> {
        ...
        return (pickNumSet.toList() + numbers.shuffled().take(6 - pickNumSet.size)).sorted()
    }
